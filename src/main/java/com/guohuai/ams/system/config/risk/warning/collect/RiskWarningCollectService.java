	...
	*/
	...
	public List<RiskWarningCollectDetResp> detail(String oid) {
		...
		for (RiskWarningCollect riskWarningCollect : list) {
			...
			temp = RiskWarningCollectDetResp.builder().title(riskWarningCollect.getRiskWarning().getIndicate().getTitle()).riskLevel(riskWarningCollect.getWlevel()).riskData(riskWarningCollect.getCollectData()).riskUnit(riskWarningCollect.getRiskWarning().getIndicate().getDataUnit()).handleLevel(riskWarningCollect.getHandleLevel()).build();
			res.add(temp);
		}
		return res;
	}
	...
	public void collect(RiskWarningCollectForm form) {
		...
		RiskWarningCollect entity = riskWarningCollectDao.findByRelativeAndRiskWarning(form.getTargetOid(), riskWarning);
		if (entity == null) {
			entity = RiskWarningCollect.builder().riskWarning(riskWarning).relative(form.getTargetOid()).collectOption(form.getOptionOid()).collectData(riskData).createTime(DateUtil.getSqlCurrentDate()).wlevel(form.getWLevel()).build();
		} else {
