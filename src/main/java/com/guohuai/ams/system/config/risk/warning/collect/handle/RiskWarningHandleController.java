...
public class RiskWarningHandleController extends BaseController {
	...
	public @ResponseBody ResponseEntity<RiskWarningHandleListResp> list(HttpServletRequest request, final String riskName, @And({ @Spec(params = "relative", path = "relative", spec = Equal.class), @Spec(params = "wlevel", path = "wlevel", spec = Equal.class) }) Specification<RiskWarningCollect> spec, @RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "50") int rows, @RequestParam(defaultValue = "desc") String sortDirection,
		...
		if (!StringUtils.isEmpty(riskName)) {
			Specification<RiskWarningCollect> nameSpec = new Specification<RiskWarningCollect>() {
				@Override
				public Predicate toPredicate(Root<RiskWarningCollect> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
					return cb.like(root.get("riskWarning").get("indicate").get("title").as(String.class), "%" + riskName + "%");
				}
			};
			spec = Specifications.where(spec).and(nameSpec);
		}
		return new ResponseEntity<RiskWarningHandleListResp>(riskWarningHandleService.list(spec, pageable), HttpStatus.OK);
	}
	...
	*/
	...
	public @ResponseBody ResponseEntity<RiskWarningHandleHisListResp> hisListAll(HttpServletRequest request, @RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "50") int rows, @RequestParam(defaultValue = "desc") String sortDirection, @RequestParam(defaultValue = "createTime") String sortField) {
		if (page < 1) {
			page = 1;
		}
		if (rows <= 0) {
		...
		Order order = new Order(Direction.valueOf(sortDirection.toUpperCase()), sortField);
		Pageable pageable = new PageRequest(page - 1, rows, new Sort(order));
		return new ResponseEntity<RiskWarningHandleHisListResp>(riskWarningHandleService.hisList(null, pageable), HttpStatus.OK);
	}
	...
	public @ResponseBody ResponseEntity<RiskWarningHandleHisListResp> hisList(HttpServletRequest request, @RequestParam(required = true) final String oid, @RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "50") int rows, @RequestParam(defaultValue = "desc") String sortDirection, @RequestParam(defaultValue = "createTime") String sortField) {
		if (page < 1) {
			page = 1;
		}
		if (rows <= 0) {
		...
		return new ResponseEntity<RiskWarningHandleHisListResp>(riskWarningHandleService.hisList(spec, pageable), HttpStatus.OK);
	}
