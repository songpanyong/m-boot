...
@Service
@Transactional
public class PublisherCollectOrderService {
	 ...
	 */
	@Transactional
	public BaseResp collect(BankOrderCollectReq bankOrderReq, String investorOid) {
		...
		/** 创建订单 */
		PublisherBankOrderEntity bankOrder = publisherBankOrderService.createCollectOrder(bankOrderReq, baseAccount);
		...
		ireq.setOrderType(AccParam.OrderType.SPVCOLLECT.toString());
		ireq.setBalance(bankOrder.getOrderAmount());
		ireq.setRemark("备注:发行人收款");
		...
		if (0 != baseRep.getErrorCode()) {
			...
			this.publisherBankOrderService.saveEntity(bankOrder);
			this.collectCallback(bankOrder);
		}
	...
	public boolean collectCallback(PublisherBankOrderEntity bankOrder) {
