...
import com.guohuai.component.exception.MoneyException;
import com.guohuai.component.util.DateUtil;
import com.guohuai.file.legal.file.LegalFileService;
import com.guohuai.mmp.investor.tradeorder.LegalTypeResp;
...
public class LegalService {
	...
	// 获取类型文件数量
	private int getFileNum(LegalEntity en) {
	...
	public LegalTypeResp<LegalType> find() {
		LegalTypeResp<LegalType> resp = new LegalTypeResp<LegalType>();
		List<LegalEntity> list = this.legalDao.findAll();
		if(list != null && list.size()>0){
			for(LegalEntity legal : list){
				LegalType  legalType = new LegalType();
				legalType.setCode(legal.getCode());
				legalType.setTypeName(legal.getName());
				resp.getList().add(legalType);
			}
		}
		return resp;
	}
