...
(
   ...
   withdrawFrozenBalance decimal(20,2) comment '提现冻结',
   rechargeFrozenBalance decimal(20,2) comment '充值冻结',
   applyAvailableBalance decimal(20,2) comment '申购冻结',
   withdrawAvailableBalance decimal(20,2) comment '提现可用',
   updateTime           timestamp not null default CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
   createTime           timestamp not null default CURRENT_TIMESTAMP,
   primary key (oid)
);
