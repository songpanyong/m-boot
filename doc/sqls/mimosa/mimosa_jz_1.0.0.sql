...
(
   ...
   manageRate           decimal(8,4),
   fixedManageRate      decimal(8,4),
   operationRate        decimal(8,4),
   accrualCycleOid      varchar(32) comment '这个是指几天发一次收益',
   payModeOid           varchar(32) comment '日: DAY
            周: WEEK
            月: MONTH',
   payModeDay           varchar(32),
   ...
   isOpenRedeemConfirm  varchar(32) comment 'YES : 允许赎回
            NO: 禁止赎回',
   accrualRepayDays     int comment '存续期结束后第()个自然日',
   purchaseConfirmDays  int comment '申购确认日:()个',
   purchaseConfirmDaysType varchar(32) comment '自然日 NATRUE
            交易日 TRADE',
   redeemConfirmDays    int comment '赎回确认日:()个',
   redeemConfirmDaysType varchar(32) comment '自然日 NATRUE
            交易日 TRADE',
   setupDateType        varchar(32) comment '固定时间 MANUALINPUT
            与首次渠道上架时间相同 FIRSTRACKTIME',
   setupDate            date comment '产品成立时间（存续期开始时间）',
   redeemTimingTaskDays int comment '赎回定时任务天数	 默认每日',
   redeemTimingTaskDaysType varchar(32) comment '赎回定时任务类型:自然日或交易日',
   redeemTimingTaskTime time comment '赎回定时任务时间 填写每日定时调支付接口做批量赎回操作的时间点',
   ...
   endDate              date,
   durationRepaymentDate date,
   stems                varchar(32) comment '前端添加 USERDEFINE
            后台添加 PLATEFORM',
   ...
   fastRedeemStatus     varchar(32) comment 'YES:是，NO:否',
   fastRedeemMax        decimal(20,4),
   fastRedeemLeft       decimal(20,4),
   ...
   isAutoAssignIncome   varchar(32) comment 'YES:是,NO:否',
   memberId             varchar(32),
   updateTime           datetime,
   createTime           datetime,
   primary key (oid)
...
/*==============================================================*/
/* Table: T_MONEY_CACHE_EXECUTE_LOG_MD                          */
/*==============================================================*/
create table T_MONEY_CACHE_EXECUTE_LOG_MD
(
   oid                  varchar(32) not null,
   batchNo              varchar(100),
   executeCommand       varchar(200),
...
/* Index: CACHE_EXECUTE_LOG_createTime                          */
/*==============================================================*/
create index CACHE_EXECUTE_LOG_createTime on T_MONEY_CACHE_EXECUTE_LOG_MD
(
   createTime
);

/*==============================================================*/
/* Index: IDX_BATCHNO_MG                                        */
/*==============================================================*/
create index IDX_BATCHNO_MG on T_MONEY_CACHE_EXECUTE_LOG_MD
(
   batchNo
);
...
(
   ...
   investWayBalance     decimal(20,2),
   onWayBalance         decimal(20,2),
   updateTime           timestamp not null default CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
   createTime           timestamp not null default CURRENT_TIMESTAMP,
   primary key (oid)
);

INSERT INTO `T_MONEY_INVESTOR_BASEACCOUNT` (`oid`, `userOid`, `memberId`, `phoneNum`, `realName`, `uid`, `status`, `balance`, `owner`, `updateTime`, `createTime`, `isFreshMan`) VALUES('superaccount','superaccount', NULL, '13888888888','超级买卖单户',NULL,'normal','0','platform','2016-08-31 11:00:01','2016-08-20 14:01:16',NULL);

/*==============================================================*/
/* Index: IDX_INVESTOR_BASEACCOUNT_USEROID                      */
/*==============================================================*/
create unique index IDX_INVESTOR_BASEACCOUNT_USEROID on T_MONEY_INVESTOR_BASEACCOUNT
(
   userOid
);
...
/*==============================================================*/
/* Index: IDX_INVESTOR_BASEACCOUNT_idNum                        */
/*==============================================================*/
create index IDX_INVESTOR_BASEACCOUNT_idNum on T_MONEY_INVESTOR_BASEACCOUNT
(
   idNum
);
...
(
   ...
   yesterdayIncome      decimal(20,4),
   toConfirmIncome      decimal(20,4),
...
/* Index: orderCode                                             */
/*==============================================================*/
create unique index orderCode on T_MONEY_INVESTOR_TRADEORDER
(
   orderCode
);

/*==============================================================*/
/* Index: pub_ostat_offset                                      */
/*==============================================================*/
create index pub_ostat_offset on T_MONEY_INVESTOR_TRADEORDER
(
   publisherOid
);

/*==============================================================*/
/* Index: investor_orderType                                    */
/*==============================================================*/
create index investor_orderType on T_MONEY_INVESTOR_TRADEORDER
(
   investorOid,
   orderType
);
...
/*==============================================================*/
/* Index: tradeOrder_uTime                                      */
/*==============================================================*/
create index tradeOrder_uTime on T_MONEY_INVESTOR_TRADEORDER
(
   updateTime
);

/*==============================================================*/
/* Index: prod_type_chan_oStat                                  */
/*==============================================================*/
create index prod_type_chan_oStat on T_MONEY_INVESTOR_TRADEORDER
(
   productOid,
   channelOid,
   orderType
...
/*==============================================================*/
create index IDX_INVESTOR_TRADEORDER_orderTime on T_MONEY_INVESTOR_TRADEORDER
(
   orderTime
);

/*==============================================================*/
/* Index: IDX_holdStatus                                        */
/*==============================================================*/
create index IDX_holdStatus on T_MONEY_INVESTOR_TRADEORDER
(
   holdStatus
);

/*==============================================================*/
/* Index: IDX_holdOid_baDate                                    */
/*==============================================================*/
create index IDX_holdOid_baDate on T_MONEY_INVESTOR_TRADEORDER
(
   holdOid,
   beginAccuralDate
);
...
/*==============================================================*/
/* Table: T_MONEY_INVESTOR_TRADEORDERQUEUE                      */
/*==============================================================*/
create table T_MONEY_INVESTOR_TRADEORDERQUEUE
(
   oid                  varchar(32) not null,
   investorOid          varchar(32) not null,
   publisherOid         varchar(32),
   ...
   channelOid           varchar(32),
   orderCode            varchar(50),
   orderType            varchar(32) comment 'invest:投资，fastRedeem:快赎，normalRedeem:普赎',
   orderAmount          decimal(20,2),
   orderVolume          decimal(20,4),
   orderTime            datetime,
   orderDate            date,
   isDone               varchar(32) comment 'yes:已处理，no:未处理',
   updateTime           timestamp not null default CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
   createTime           timestamp default CURRENT_TIMESTAMP,
   primary key (oid)
);
...
/* Index: orderCode                                             */
/*==============================================================*/
create unique index orderCode on T_MONEY_INVESTOR_TRADEORDERQUEUE
(
   orderCode
);
...
/* Index: pub_ostat_offset                                      */
/*==============================================================*/
create index pub_ostat_offset on T_MONEY_INVESTOR_TRADEORDERQUEUE
(
   publisherOid
);
...
/* Index: investor_orderType                                    */
/*==============================================================*/
create index investor_orderType on T_MONEY_INVESTOR_TRADEORDERQUEUE
(
   investorOid,
   orderType
);
...
/* Index: tradeOrder_uTime                                      */
/*==============================================================*/
create index tradeOrder_uTime on T_MONEY_INVESTOR_TRADEORDERQUEUE
(
   updateTime
);
...
/* Index: prod_type_chan_oStat                                  */
/*==============================================================*/
create index prod_type_chan_oStat on T_MONEY_INVESTOR_TRADEORDERQUEUE
(
   productOid,
   channelOid,
   orderType
...
/* Index: Index_1                                               */
/*==============================================================*/
create index Index_1 on T_MONEY_PLATFORM_FINANCE_MODIFYORDER
(
   orderCode,
   tradeType
);
...
/* Index: Index_2                                               */
/*==============================================================*/
create index Index_2 on T_MONEY_PLATFORM_FINANCE_MODIFYORDER
(
   tradeType
);
...
/* Index: Index_3                                               */
/*==============================================================*/
create index Index_3 on T_MONEY_PLATFORM_FINANCE_MODIFYORDER
(
   tradeType
);
...
/* Index: Index_4                                               */
/*==============================================================*/
create index Index_4 on T_MONEY_PLATFORM_FINANCE_MODIFYORDER
(
   updateTime,
   tradeType
);
...
/*==============================================================*/
create unique index productOid_offsetCode on T_MONEY_PLATFORM_PUBLISHER_PRODUCT_OFFSET
(
   productOid,
   offsetCode
);
...
(
   oid                  varchar(32) not null,
   memberId             varchar(32),
   corperateOid         varchar(32),
   bankCode             varchar(32),
   accountBalance       decimal(20,4),
...
/*==============================================================*/
/* Table: T_MONEY_PUBLISHER_INVESTOR_HOLDINCOME_MD              */
/*==============================================================*/
create table T_MONEY_PUBLISHER_INVESTOR_HOLDINCOME_MD
(
   oid                  varchar(32) not null,
   holdOid              varchar(32),
   productOid           varchar(32),
...
/* Index: hold_invest_dt_prod                                   */
/*==============================================================*/
create index hold_invest_dt_prod on T_MONEY_PUBLISHER_INVESTOR_HOLDINCOME_MD
(
   investorOid,
   confirmDate,
   productOid
...
(
   ...
   investorOid          varchar(32),
   orderOid             varchar(32),
   snapshotVolume       decimal(20,4),
   snapShotDate         date comment 'in:进，out:出',
   redeemStatus         varchar(32),
   holdDays             int,
   updateTime           timestamp not null default CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
   createTime           timestamp not null default CURRENT_TIMESTAMP,
   primary key (oid)
...
/*==============================================================*/
/* Index: IDX_snapshot_orderOid_ssDate                          */
/*==============================================================*/
create index IDX_snapshot_orderOid_ssDate on T_MONEY_PUBLISHER_INVESTOR_HOLD_SNAPSHOT
(
   orderOid,
   snapShotDate
);
...
/*==============================================================*/
/* Index: IDX_snapshot_snapshotDate                             */
/*==============================================================*/
create index IDX_snapshot_snapshotDate on T_MONEY_PUBLISHER_INVESTOR_HOLD_SNAPSHOT
(
   snapshotVolume
);
...
/*==============================================================*/
/* Table: T_MONEY_PUBLISHER_INVESTOR_INCOME_MD                  */
/*==============================================================*/
create table T_MONEY_PUBLISHER_INVESTOR_INCOME_MD
(
   oid                  varchar(32) not null,
   holdOid              varchar(32),
   productOid           varchar(32),
...
/* Index: prod_hOid_cDt                                         */
/*==============================================================*/
create index prod_hOid_cDt on T_MONEY_PUBLISHER_INVESTOR_INCOME_MD
(
   productOid,
   confirmDate,
   holdOid
...
/* Index: haIncome_uTime                                        */
/*==============================================================*/
create index haIncome_uTime on T_MONEY_PUBLISHER_INVESTOR_INCOME_MD
(
   updateTime
);
...
/*==============================================================*/
/* Table: T_MONEY_PUBLISHER_INVESTOR_LEVELINCOME_MD             */
/*==============================================================*/
create table T_MONEY_PUBLISHER_INVESTOR_LEVELINCOME_MD
(
   oid                  varchar(32) not null,
   holdOid              varchar(32),
   productOid           varchar(32),
...
/* Index: li_uTime                                              */
/*==============================================================*/
create index li_uTime on T_MONEY_PUBLISHER_INVESTOR_LEVELINCOME_MD
(
   updateTime
);
...
/* Index: invest_prod_rRule                                     */
/*==============================================================*/
create index invest_prod_rRule on T_MONEY_PUBLISHER_INVESTOR_LEVELINCOME_MD
(
   investorOid,
   productOid,
   rewardRuleOid
...
(
   ...
   productOid           varchar(32),
   rewardRuleOid        varchar(32),
   totalHoldVolume      decimal(20,2),
   totalRewardIncome    decimal(20,2) comment 'on:开，off:关',
   tDate                date,
   updateTime           timestamp default CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
   createTime           timestamp default CURRENT_TIMESTAMP,
   primary key (oid)
...
(
   ...
   accrualCycleType     varchar(32) comment '枚举值
            自然年: NATURAL_YEAR
            合同年: CONTRACT_YEAR
            ',
   ...
   repayment            longtext,
   risk                 longtext,
   servicePlatform      varchar(32),
   regulatoryBank       varchar(32),
   recordOrganization   varchar(32),
